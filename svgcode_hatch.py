...
# 首先使用同样的方式分解选中的图形为单独的moveto和lineto坐标。只保留多边形的顶点列表。
# 一个图形可能会被多个多边形子图形来表示
#
...
# 有了顶点列表，就可以用所有点的边界画出hatchline。每个hatchline与多边形的边界交点就可知。
# 每个交点使用[0,1]的小数来存储，表示他们在hatchline上的位置。0就是在hatchline开头，1就是在末尾，0.5则在中间
# 
...
# another.
# 当两个多变形一边重合的时候，hatchline也会在边界处重复画，此时要移除重复的hatchline
#
...
# 一但排序并移除了重复的，用奇偶规则决定哪个hatchline应用在多边形内部。这些在内部的hatchline会被画
#
...
# 每保存一个hatchline，也会保存他对应的SVG元素。这样就可以根据关联的元素进行分类
# 这样就可以操作两个图形和hatchline在Inkscape中作为一个对象
#
...
# graphical element.
# 每个图形都会保存变化矩阵，方便复原。为了计算hatch填充，先把所有的图形进行变化，来知道每个元素的相对位置。
# 计算完hatchline后，需要逆变换把hatchline变会原来的坐标系
...
2. The lines are coincident d = 0 and the numerators for f & sb are zero
...
def intersect(p1, p2, p3, p4):
    ...
    intersection occurs.
    判断两个线段是否相交，如果相交则返回sa在第一条线上的长度比例
    """
    ...
    # 第一个线段是长的hatchline，线段二肯定比较短，所及计算线段二更有可能遇到非交集的情况
